...
class UserIdSkill(MycroftSkill):
    ...
    def handle_who_am_i_intent(self, message):
        ...
        if "fbchat" in user_id:
            ...
            vision_user = face_recog.face_recognition_from_url(url, self.context, server=False)
        else:
            vision = VisionService(self.emitter)
            feed = vision.get_feed(self.context)
            vision_user = face_recog.face_recognition_from_file(feed, self.context, server=False)
