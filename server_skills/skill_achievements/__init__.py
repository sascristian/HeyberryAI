...
class AchievementsSkill(MycroftSkill):
    ...
    def add_standard_achievements(self):
        self.add_achievement("mathematical art", {"timestamp": None, "text": "Made art plotting equations",
        ...
        self.add_achievement("face detection", {"timestamp": None, "text": "Detected faces using haar cascades!",
                                                "url": "https://pythonprogramming.net/haar-cascade-object-detection-python-opencv-tutorial/"})
        self.add_achievement("joke",
                             {"timestamp": None, "text": "Said a joke", "url": "https://github.com/pyjokes/pyjokes"})
        ...
        self.add_achievement("tts",
                             {"timestamp": None, "text": "i speak therefore i am", "url": "https://mimic.mycroft.ai/"})
        self.add_achievement("deep dream", {"timestamp": None, "text": "deep dreaming performed",
        ...
        self.add_achievement("deep draw", {"timestamp": None, "text": "deep draw performed",
    ...
    def handle_new_achievement(self, message):
        ...
        if "JokingIntent" in type:
            ...
            name = "deep dream"
        elif "inspirobot.result" in type:
            name = "inspirobot"
        elif "object.recognition.result" in type:
            name = "tensor flow object recognition"
        elif "art.result" in type:
            name = "mathematical art"
        elif "tweet.post" in type:
            name = "tweet"
        else:
            return
        self.add_achievement(name)
        self.log.info("Achievement unlocked " + str(self.last_achievement))
        self.emitter.emit(Message("achievement", {"name": self.last_achievement,
                                                  "achievement": self.settings["achievements"][self.last_achievement]},
                                  self.context))
        if not self.settings["achievements"][name].get("tweeted", False):
            text = self.settings["achievements"][self.last_achievement]["text"] + " " + self.settings["achievements"][
        ...
        if self.settings["achievements"][name]["count"] % 100 == 0:
            text = str(self.settings["achievements"][name]["count"]) + " times " + \
                   self.settings["achievements"][self.last_achievement]["text"] + " " + self.settings["achievements"][
                       self.last_achievement].get("url", "")
            self.tweet_request(text + " #AchievementUnlocked")
    ...
    def tweet_request(self, text):
        ...
        tweet_text = text
        self.emitter.emit(
            Message("tweet_request", {"tweet_type": tweet_type, "tweet_pic": tweet_pic, "tweet_text": tweet_text},
                    self.context))
