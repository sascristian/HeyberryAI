        ...
        for n in range(len(datenames)):
            try:
                launch = dict(launch_date=datenames[n].find('span', attrs={'class': 'launchdate'}).text,
                              ...
                              launch_time=missiondatas[n].text.split("\n")[0].replace("Launch window: ", "").replace(
                                  "Launch time: ", ""),
                              launch_location=missiondatas[n].text.split("\n")[1].replace("Launch site: ", ""))
                launch['launch_date'] = launch['launch_date'].replace("Sept.",
                                                                      "Sep.")  # For some reason the parser doesn't like Sept
                ...
                launch['launch_time'] = re.search(r"(\d+:.+EDT)", launch['launch_time']).group(0).replace("EDT",
                                                                                                          "Eastern Daylight Time").replace(
