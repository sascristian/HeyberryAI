...
import time
import traceback
from jarbas_models.tf_tacotron.text import cmudict, text_to_sequence
...
class DataFeeder(threading.Thread):
    ...
    def __init__(self, coordinator, metadata_filename, hparams):
        ...
        with open(metadata_filename) as f:
            ...
            hours = sum((int(x[2]) for x in
                         self._metadata)) * hparams.frame_shift_ms / (
                    3600 * 1000)
        ...
        if hparams.use_cmudict:
            ...
            log('Loaded CMUDict with %d unambiguous entries' % len(
                self._cmudict))
        else:
            self._cmudict = None
    ...
    def _enqueue_next_group(self):
        ...
        examples = [self._get_next_example() for i in
                    range(n * _batches_per_group)]
        ...
        for batch in batches:
            feed_dict = dict(
                zip(self._placeholders, _prepare_batch(batch, r)))
    ...
    def _maybe_get_arpabet(self, word):
        ...
        return '{%s}' % arpabet[
            0] if arpabet is not None and random.random() < 0.5 else word
