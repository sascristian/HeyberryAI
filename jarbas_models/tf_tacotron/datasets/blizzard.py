...
import numpy as np
import os
from jarbas_models.tf_tacotron.hparams import hparams
from jarbas_models.tf_tacotron.util import audio
    ...
    for book in books:
        with open(os.path.join(in_dir, book, 'sentence_index.txt')) as f:
            for line in f:
                ...
                if line[0] is not '#' and len(parts) == 8 and float(
                        parts[3]) > _min_confidence:
                    wav_path = os.path.join(in_dir, book, 'wav',
                                            '%s.wav' % parts[0])
                    labels_path = os.path.join(in_dir, book, 'lab',
                                               '%s.lab' % parts[0])
                    text = parts[5]
                    task = partial(_process_utterance, out_dir, index,
                                   wav_path, labels_path, text)
...
def _process_utterance(out_dir, index, wav_path, labels_path, text):
    ...
    start = int(start_offset * hparams.sample_rate)
        ...
        end_offset * hparams.sample_rate) if end_offset is not None else -1
    wav = wav[start:end]
