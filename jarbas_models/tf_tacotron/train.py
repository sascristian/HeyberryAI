...
from jarbas_models.tf_tacotron.datasets.datafeeder import DataFeeder
from jarbas_models.tf_tacotron.hparams import hparams, hparams_debug_string
from jarbas_models.tf_tacotron.models import create_model
from jarbas_models.tf_tacotron.text import sequence_to_text
from jarbas_models.tf_tacotron.util import audio, infolog, plot, ValueWindow
...
def get_git_commit():
    subprocess.check_output(
        ['git', 'diff-index', '--quiet', 'HEAD'])  # Verify client is clean
    ...
    with tf.Session() as sess:
        try:
            ...
            if args.restore_step:
                ...
                restore_path, commit), slack=True)
            else:
            ...
            while not coord.should_stop():
                ...
                if step % args.checkpoint_interval == 0:
                    ...
                    audio.save_wav(waveform, os.path.join(log_dir,
                                                          'step-%d-audio.wav' % step))
                    plot.plot_alignment(alignment, os.path.join(log_dir,
                                                                'step-%d-align.png' % step),
                                        info='%s, %s, %s, step=%d, loss=%.5f' % (
                                        args.model, commit, time_string(),
                                        step, loss))
