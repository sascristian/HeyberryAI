...
tmp_dict = {chr(i): ' ' for i in range(256) if
            chr(i) not in _valid_input_chars}
_trans_table = string.maketrans(''.join(tmp_dict.keys()),
                                ''.join(tmp_dict.values()))
...
_arpabet_to_id = {sym: i + _num_normal_symbols for i, sym in
                  enumerate(cmudict.valid_symbols)}
_id_to_arpabet = {i + _num_normal_symbols: sym for i, sym in
                  enumerate(cmudict.valid_symbols)}
...
def to_sequence(text, force_lowercase=True, expand_abbreviations=True):
    ...
    while len(text):
        ...
        if not m:
            sequence += _text_to_sequence(text, force_lowercase,
                                          expand_abbreviations)
            break
        sequence += _text_to_sequence(m.group(1), force_lowercase,
                                      expand_abbreviations)
...
_abbreviations = [(re.compile('\\b%s\\.' % x[0], re.IGNORECASE), x[1]) for x
                  in [
                      ('mrs', 'misess'),
                      ('mr', 'mister'),
                      ('dr', 'doctor'),
