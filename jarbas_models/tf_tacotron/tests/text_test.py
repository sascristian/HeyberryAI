from jarbas_models.tf_tacotron.text import cleaners, symbols, \
...
def test_text_to_sequence():
    ...
    assert text_to_sequence('A {AW1 S}  B', ['english_cleaners']) == [28, 64,
                                                                      83, 132,
                                                                      64, 29,
                                                                      1]
...
def test_expand_abbreviations():
    assert cleaners.expand_abbreviations(
        'mr. and mrs. smith') == 'mister and misess smith'


def test_expand_numbers():
    assert cleaners.expand_numbers(
        '3 apples and 44 pears') == 'three apples and forty-four pears'
    assert cleaners.expand_numbers(
        '$3.50 for gas.') == 'three dollars, fifty cents for gas.'


def test_cleaner_pipelines():
    text = 'Mr. Müller ate  2 Apples'
    assert cleaners.english_cleaners(text) == 'mister muller ate two apples'
    assert cleaners.transliteration_cleaners(
        text) == 'mr. muller ate 2 apples'
