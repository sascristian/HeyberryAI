...
class ArticleSkill(MycroftSkill):
    ...
    def initialize(self):
        ...
        fb_article_intent = IntentBuilder("FbArticleIntent") \
            .require("fbarticle").build()
        self.register_intent(fb_article_intent,
                             self.handle_fb_article_intent)
    ...
    def handle_fb_article_intent(self, message):
        news = newspaper.build(random.choice(self.newssauce), memoize_articles=False)
        # print cnn_paper.feed_urls()
        ...
        message += "\n\n"
        # process text
        message.replace("Read More", "")
        message.replace("Comment this news or article", "")
        message.replace("Photo", "")
        self.poster.post_link(link=article.url, text=message, speech="Sharing an article on face book")
