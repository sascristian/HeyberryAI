...
class freewill():
    ...
    def __init__(self):
        ...
        self.generic = ['awesome', 'cool', 'the best', 'brutal', 'average', 'satanic', 'synthetic', "nice", "ultra", "strange","weird"]
        self.entropy = []
        self.activefriends = []
        self.sentiment = ""
        self.mood = "neutral"
        ...
        def increasedopamine(message):
            x = message.data.get('ammount')
            self.context.dopamine += x

        def increaseserotonine(message):
            x = message.data.get('ammount')
            self.context.serotonine += x

        def decreasedopamine(message):
            x = message.data.get('ammount')
            self.context.dopamine -= x

        def decreaseserotonine(message):
            x = message.data.get('ammount')
            self.context.serotonine -= x

        def entropyate(message):
            flag = False
        ...
        client.emitter.on('serotonine_increase_request', increaseserotonine)
        client.emitter.on('serotonine_decrease_request', decreaseserotonine)
        client.emitter.on('entropy_update', entropyate)
        client.emitter.on('entropy_request', entropy_request)
        client.emitter.on('entropy_reset', entropy_reset)
