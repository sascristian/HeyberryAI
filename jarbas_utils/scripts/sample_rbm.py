...
def dedupe_cb(strings, i, energy=None):
    ...
    for i in range(len(strings)):
        if strings[i] in DEDUPE_SEEN[i]:
            continue
        print strings[i] + "\t" + (
        "{:.2f}".format(energy[i]) if energy is not None else "")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--start-temp', dest='start_temp', type=float,
                        default=1.0, help="Temperature for first iteration")
    parser.add_argument('-e', '--end-temp', dest='end_temp', type=float,
                        default=1.0, help="Temperature at last iteration")
    ...
    for model_fname in args.model_fname:
        ...
        kwargs = dict(start_temp=args.start_temp, final_temp=args.end_temp,
        ...
        vis = Sampling.sample_model(model, args.n_samples, args.iters,
